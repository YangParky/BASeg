...
def get_parser():
    ...
    parser.add_argument('--config', type=str, default='config/cityscapes/camvid_baseg101.yaml', help='config file')
    parser.add_argument('opts', help='see config/ade20k/camvid_baseg101.yaml for all options', default=None,
...
def main_worker(gpu, ngpus_per_node, argss):
    ...
    if args.arch == 'baseg':
        from model.baseg_finetune import BASeg
        model = BASeg(layers=args.layers, num_classes=args.classes, BatchNorm=BatchNorm, in_channels=args.in_channels,
                      embed_dim=args.embed_dim, depth=args.depth, criterion=criterion, multi_grid=tuple(args.multi_grid),
                      pretrained=True)
    ...
    if args.distributed:
        ...
        model = torch.nn.parallel.DistributedDataParallel(model, device_ids=[gpu], broadcast_buffers=False)
    else:
        model, optimizer = amp.initialize(model.cuda(), optimizer, opt_level='O1')
    ...
    if args.weight:
        if os.path.isfile(args.weight):
            ...
            model_dict = {k:v for k,v in checkpoint.items() if k in state_dict.keys()}
    ...
    train_data = dataset.CityscapesEdgeData(split='train', num_classes=args.classes, data_root=args.data_root,
    ...
    if args.evaluate:
        ...
        val_data = dataset.CityscapesEdgeData(split='val', num_classes=args.classes, data_root=args.data_root,
